...
const url = ['https://www.naarstore.com/']
...
export default async function naarstore({ page, enqueueLinks, request, log, addRequests }) {
    debugger
...
async function getUrls(page, addRequests) {
    ...
    if (nextPage) {
        ...
        const totalPages = await page.evaluate(() => Math.ceil(parseInt( document.querySelector('#paginationinfo').innerText.split(' ')[0])/100) )
